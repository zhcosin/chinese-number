...
(defun cnum-get-chinese-number-and-weight-high (number weight-index)
  (cnum-get-chinese-number-and-weight-general number weight-index 'cnum-chinese-small-number 'cnum-get-chinese-high-level-weight))
...
(defun cnum-chinese-number-general-iter (number index next-is-zero base fun-to-convert-small-number iter-depth)
  (let ((real-number (% number base)))
  (if (< number base)
      (funcall fun-to-convert-small-number number index (if (= iter-depth 0) nil next-is-zero))
    (concat (cnum-chinese-number-general-iter (/ number base) (+ index 1) (= real-number 0) base fun-to-convert-small-number (+ iter-depth 1))
	    (funcall fun-to-convert-small-number real-number index next-is-zero)))))

(defun cnum-chinese-small-number-iter (number index next-is-zero)
  (cnum-chinese-number-general-iter number index next-is-zero 10 'cnum-get-chinese-number-weight 0))

(defun cnum-chinese-high-number-iter (number index next-is-zero)
  (cnum-chinese-number-general-iter number index next-is-zero 10000 'cnum-get-chinese-number-and-weight-high 0)
  )

(defun cnum-chinese-small-number (number)
  (cnum-chinese-small-number-iter number 0 t))

(defun cnum-chinese-high-number (number)
